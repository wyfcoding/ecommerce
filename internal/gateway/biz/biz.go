...
// GatewayUsecase is the business logic for the API Gateway.
type GatewayUsecase struct {
	authClient AuthClient
	// TODO: Add clients for other microservices (Product, Order, User, etc.)
}

// NewGatewayUsecase creates a new GatewayUsecase.
func NewGatewayUsecase(authClient AuthClient) *GatewayUsecase {
	return &GatewayUsecase{authClient: authClient}
}

// HandleRequest handles a generic incoming request, including authentication and routing.
func (uc *GatewayUsecase) HandleRequest(ctx context.Context, method, path, body, accessToken string, headers map[string]string) (statusCode int32, responseBody string, responseHeaders map[string]string, err error) {
	// 1. Authentication (if access token is provided)
	if accessToken != "" {
		isValid, userID, username, authErr := uc.authClient.ValidateToken(ctx, accessToken)
		if authErr != nil {
