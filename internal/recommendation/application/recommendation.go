...
// 它协调 RecommendationCommandService 和 RecommendationQueryService 处理用户推荐获取、追踪、偏好更新和推荐生成。
type RecommendationService struct {
	Command *RecommendationCommandService
	Query   *RecommendationQueryService
	Logger  *slog.Logger
}

// NewRecommendationService 创建并返回一个新的 RecommendationService 实例。
func NewRecommendationService(command *RecommendationCommandService, query *RecommendationQueryService, logger *slog.Logger) *RecommendationService {
...
func (s *RecommendationService) TrackBehavior(ctx context.Context, userID, productID uint64, action string) error {
	...
	return s.Command.SaveUserBehavior(ctx, behavior)
}
...
func (s *RecommendationService) UpdateUserPreference(ctx context.Context, pref *domain.UserPreference) error {
	...
	return s.Command.SaveUserPreference(ctx, pref)
}
...
func (s *RecommendationService) GenerateRecommendations(ctx context.Context, userID uint64) error {
	...
	// 2. 清除旧的推荐数据。
	if err := s.Command.DeleteRecommendations(ctx, userID, nil); err != nil {
		return err
	}
	...
	// 4. 保存新生成的推荐数据。
	for _, r := range recs {
		if err := s.Command.SaveRecommendation(ctx, r); err != nil {
