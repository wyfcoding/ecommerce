...
type PricingRepo interface {
	GetPriceRulesForSKUs(ctx context.Context, skuIDs []uint64) ([]*PriceRule, error)
	SaveDiscount(ctx context.Context, discount *Discount) (*Discount, error)
}
...
// PricingUsecase is the business logic for pricing.
...
// NewPricingUsecase creates a new PricingUsecase.
func NewPricingUsecase(repo PricingRepo, aiModelClient AIModelClient) *PricingUsecase {
	return &PricingUsecase{repo: repo, aiModelClient: aiModelClient}
}

// CalculateFinalPrice calculates the final price for a list of SKUs, considering rules and discounts.
func (uc *PricingUsecase) CalculateFinalPrice(ctx context.Context, userID uint64, items []*SkuPriceInfo, couponCode string) (totalOriginalPrice, totalDiscountAmount, finalPrice uint64, err error) {
	totalOriginalPrice = 0
	totalDiscountAmount = 0

	// 1. Calculate total original price and apply base/special pricing rules
	skuIDs := make([]uint64, len(items))
	itemMap := make(map[uint64]*SkuPriceInfo)
	...
	priceRules, err := uc.repo.GetPriceRulesForSKUs(ctx, skuIDs)
	if err != nil {
		return 0, 0, 0, err
	}
	...
	if couponCode != "" {
		...
		if priceAfterRules >= simulatedCouponDiscount {
			...
			_, err = uc.repo.SaveDiscount(ctx, &Discount{
				DiscountType: "COUPON",
...
func (uc *PricingUsecase) CalculateDynamicPrice(ctx context.Context, productID, userID uint64, contextFeatures map[string]string) (uint64, string, error) {
	// Call AI Model Service to get dynamic price
	dynamicPrice, explanation, err := uc.aiModelClient.CalculateDynamicPrice(ctx, productID, userID, contextFeatures)
	if err != nil {
