...
type SettlementService struct {
	repo            domain.SettlementBridgeRepository
	treasurySvc     domain.TreasuryService
	paymentSvc      domain.PaymentService
	eventSubscriber messagequeue.EventSubscriber
	logger          *slog.Logger
}
...
func (s *SettlementService) StartEventListening(ctx context.Context) error {
	// 监听电商支付成功事件
	return s.eventSubscriber.Subscribe(ctx, "payment.succeeded", s.handlePaymentSucceeded)
}

// handlePaymentSucceeded 处理支付成功事件
func (s *SettlementService) handlePaymentSucceeded(ctx context.Context, event eventsourcing.DomainEvent) error {
	base, ok := event.(*eventsourcing.BaseEvent)
	if !ok {
		return fmt.Errorf("event is not *BaseEvent")
	}

	data, ok := base.Data.(map[string]interface{})
	if !ok {
		return fmt.Errorf("event data is not map[string]interface{}")
	}
