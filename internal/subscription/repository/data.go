...
var ProviderSet = wire.NewSet(NewData, NewSubscriptionRepo)
...
// NewSubscriptionRepo is a provider function that creates a new subscription repository.
// It depends on the Data struct (which has the db connection).
func NewSubscriptionRepo(data *Data) biz.SubscriptionRepo {
	// The actual implementation is in subscription.go
