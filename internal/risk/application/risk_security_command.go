...
import (
	...
	finance "github.com/wyfcoding/pkg/algos/finance"
	graph "github.com/wyfcoding/pkg/algos/graph"
...
// RemoveFromBlacklistByTypeAndValue 从黑名单中移除指定类型和值的实体。
func (m *RiskSecurityCommandService) RemoveFromBlacklistByTypeAndValue(ctx context.Context, bType string, value string) error {
	if err := m.repo.DeleteBlacklistByTypeAndValue(ctx, domain.BlacklistType(bType), value); err != nil {
		m.logger.ErrorContext(ctx, "failed to remove from blacklist", "type", bType, "value", value, "error", err)
		return err
	}
	m.logger.InfoContext(ctx, "removed from blacklist", "type", bType, "value", value)
	return nil
}
