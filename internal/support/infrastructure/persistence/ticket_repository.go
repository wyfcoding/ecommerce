...
// NewTicketRepository 创建并返回一个新的 customerRepository 实例。
...
func (r *customerRepository) SaveConversation(ctx context.Context, conversation *domain.Conversation) error {
	return r.db.WithContext(ctx).Save(conversation).Error
}

func (r *customerRepository) GetConversation(ctx context.Context, id uint64) (*domain.Conversation, error) {
...
func (r *customerRepository) SaveConversationMessage(ctx context.Context, message *domain.ConversationMessage) error {
	return r.db.WithContext(ctx).Save(message).Error
}

func (r *customerRepository) ListConversationMessages(ctx context.Context, conversationID uint64, offset, limit int) ([]*domain.ConversationMessage, int64, error) {
	var list []*domain.ConversationMessage
	var total int64

	db := r.db.WithContext(ctx).Model(&domain.ConversationMessage{}).Where("conversation_id = ?", conversationID)
