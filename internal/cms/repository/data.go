...
var ProviderSet = wire.NewSet(NewData, NewCmsRepo)
...
// NewCmsRepo is a provider function that creates a new CMS repository.
// It depends on the Data struct (which has the db connection).
func NewCmsRepo(data *Data) biz.CmsRepo {
	// The actual implementation is in cms.go
	return &cmsRepo{
		data: data,
	}
}
