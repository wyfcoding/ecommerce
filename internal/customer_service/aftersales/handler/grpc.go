...
// 它监听指定的地址和端口，并注册 AftersalesServiceServer。
func StartGRPCServer(grpcService *service.AftersalesGRPCService, addr string, port int) (*grpc.Server, chan error) {
	...
	// 创建新的 gRPC 服务器
	s := grpc.NewServer()
	// 注册 AftersalesServiceServer
	v1.RegisterAftersalesServiceServer(s, grpcService)
