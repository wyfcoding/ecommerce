...
func (s *Server) ListReturnRequests(ctx context.Context, req *pb.ListReturnRequestsRequest) (*pb.ListReturnRequestsResponse, error) {
	...
	if req.Status != nil {
		...
		st := domain.AfterSalesStatus(*req.Status)
		query.Status = st
	}
...
func (s *Server) UpdateSupportTicketStatus(ctx context.Context, req *pb.UpdateSupportTicketStatusRequest) (*pb.SupportTicketResponse, error) {
	var st domain.SupportTicketStatus
	switch req.Status {
	...
	case pb.SupportTicketStatus_SUPPORT_TICKET_STATUS_IN_PROGRESS:
		st = domain.SupportTicketStatusPending // 将 InProgress 映射为 Pending 或类似状态
	case pb.SupportTicketStatus_SUPPORT_TICKET_STATUS_RESOLVED:
		st = domain.SupportTicketStatusResolved
	case pb.SupportTicketStatus_SUPPORT_TICKET_STATUS_CLOSED:
		st = domain.SupportTicketStatusClosed
	default:
		st = domain.SupportTicketStatusOpen
	}
...
func (s *Server) ListSupportTickets(ctx context.Context, req *pb.ListSupportTicketsRequest) (*pb.ListSupportTicketsResponse, error) {
	...
	if req.Status != nil && *req.Status != pb.SupportTicketStatus_SUPPORT_TICKET_STATUS_UNSPECIFIED {
		...
		switch *req.Status {
		case pb.SupportTicketStatus_SUPPORT_TICKET_STATUS_OPEN:
			sVal = int(domain.SupportTicketStatusOpen)
		case pb.SupportTicketStatus_SUPPORT_TICKET_STATUS_IN_PROGRESS:
			sVal = int(domain.SupportTicketStatusPending)
		case pb.SupportTicketStatus_SUPPORT_TICKET_STATUS_RESOLVED:
			sVal = int(domain.SupportTicketStatusResolved)
		case pb.SupportTicketStatus_SUPPORT_TICKET_STATUS_CLOSED:
			sVal = int(domain.SupportTicketStatusClosed)
		default:
			sVal = int(domain.SupportTicketStatusOpen)
		}
		statusPtr = &sVal
...
func (s *Server) toProto(as *domain.AfterSales) *pb.ReturnRequest {
	// 映射领域实体状态到protobuf状态。
	var status pb.ReturnRequestStatus
	switch as.Status {
	case domain.AfterSalesStatusPending:
		status = pb.ReturnRequestStatus_RETURN_REQUEST_STATUS_PENDING
	case domain.AfterSalesStatusApproved:
...
func (s *Server) toSupportTicketMessageProto(m *domain.SupportTicketMessage) *pb.SupportTicketMessage {
	if m == nil {
		return nil
	}
	isAdmin := false
