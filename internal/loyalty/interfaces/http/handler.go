...
func NewHandler(cmd *application.LoyaltyCommandService, query *application.LoyaltyQueryService, logger *slog.Logger) *Handler {
	return &Handler{
		cmd:    cmd,
		query:  query,
		logger: logger,
...
func (h *Handler) GetAccount(c *gin.Context) {
	...
	account, err := h.query.GetOrCreateAccount(c.Request.Context(), userID)
	if err != nil {
...
func (h *Handler) UpdatePoints(c *gin.Context) {
	...
	if req.Action == "add" {
		opErr = h.cmd.AddPoints(ctx, userID, req.Points, req.Type, req.Description, req.OrderID)
	} else {
		opErr = h.cmd.DeductPoints(ctx, userID, req.Points, req.Type, req.Description, req.OrderID)
	}
...
func (h *Handler) GetTransactions(c *gin.Context) {
	...
	list, total, err := h.query.GetPointsTransactions(c.Request.Context(), userID, page, pageSize)
	if err != nil {
...
func (h *Handler) AddBenefit(c *gin.Context) {
	...
	benefit, err := h.cmd.AddBenefit(c.Request.Context(), domain.MemberLevel(req.Level), req.Name, req.Description, req.DiscountRate, req.PointsRate)
	if err != nil {
...
func (h *Handler) ListBenefits(c *gin.Context) {
	level := c.Query("level")
	list, err := h.query.ListBenefits(c.Request.Context(), domain.MemberLevel(level))
	if err != nil {
...
func (h *Handler) DeleteBenefit(c *gin.Context) {
	...
	if err := h.cmd.DeleteBenefit(c.Request.Context(), id); err != nil {
