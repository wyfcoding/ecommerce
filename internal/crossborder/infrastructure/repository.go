...
func NewCrossBorderRepository(db *gorm.DB) *CrossBorderRepository {
...
func (r *CrossBorderRepository) GetDeclaration(ctx context.Context, id string) (*domain.CustomsDeclaration, error) {
	var d domain.CustomsDeclaration
	if err := r.db.WithContext(ctx).Preload("Items").Where("declaration_id = ?", id).First(&d).Error; err != nil {
		return nil, err
	}
	return &d, nil
}
