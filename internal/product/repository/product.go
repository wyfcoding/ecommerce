...
func NewProductRepo(data *Data, mongoClient *mongo.Client, esClient *elasticsearch.Client) biz.ProductRepo {
...
func (r *productRepo) SaveProductToElasticsearch(ctx context.Context, spu *biz.Spu) error {
	...
	res, err := r.esClient.Index(
		"products", // Assuming "products" index in Elasticsearch
		bytes.NewReader(data),
		r.esClient.Index.WithDocumentID(strconv.FormatUint(spu.ID, 10)),
	...
	if res.IsError() {
		return fmt.Errorf("failed to index product to Elasticsearch: %s", res.String())
	}
