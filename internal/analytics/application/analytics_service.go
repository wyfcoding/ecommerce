...
// AnalyticsService 结构体定义了数据分析相关的应用服务 (外观模式)。
...
func NewAnalyticsService(manager *AnalyticsManager, query *AnalyticsQuery, idGenerator idgen.Generator) *AnalyticsService {
...
func (s *AnalyticsService) RecordMetric(ctx context.Context, metricType domain.MetricType, name string, value float64, granularity domain.TimeGranularity, dimension, dimensionVal string) error {
...
func (s *AnalyticsService) QueryMetrics(ctx context.Context, query *domain.MetricQuery) ([]*domain.Metric, int64, error) {
	return s.query.SearchMetrics(ctx, query)
}

// CreateDashboard 创建一个新的仪表板。
func (s *AnalyticsService) CreateDashboard(ctx context.Context, name, description string, userID uint64) (*domain.Dashboard, error) {
...
func (s *AnalyticsService) GetDashboard(ctx context.Context, id uint64) (*domain.Dashboard, error) {
	return s.query.GetDashboardByID(ctx, id)
}

// AddMetricToDashboard 将一个指标添加到指定的仪表板。
func (s *AnalyticsService) AddMetricToDashboard(ctx context.Context, dashboardID uint64, metricType domain.MetricType, title, chartType string) error {
...
func (s *AnalyticsService) CreateReport(ctx context.Context, title, description string, userID uint64, reportType string) (*domain.Report, error) {
...
func (s *AnalyticsService) GetReport(ctx context.Context, id uint64) (*domain.Report, error) {
