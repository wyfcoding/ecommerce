...
import (
	...
	"github.com/wyfcoding/ecommerce/internal/customerservice/domain"
)
...
func (m *CustomerServiceManager) CreateTicket(ctx context.Context, userID uint64, subject, description, category string, priority domain.TicketPriority) (*domain.Ticket, error) {
...
// ReplyTicket 回复一个工单。
func (m *CustomerServiceManager) ReplyTicket(ctx context.Context, ticketID, senderID uint64, senderType, content string, msgType domain.MessageType) (*domain.Message, error) {
...
// CloseTicket 关闭一个工单。
func (m *CustomerServiceManager) CloseTicket(ctx context.Context, id uint64) error {
...
func (m *CustomerServiceManager) ResolveTicket(ctx context.Context, id uint64) error {
	ticket, err := m.repo.GetTicket(ctx, id)
	if err != nil {
		return err
	}
