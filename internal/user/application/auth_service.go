...
// Register 注册一个新用户
func (s *AuthService) Register(ctx context.Context, username, password, email, phone string) (uint64, error) {
	// 1. 检查用户名是否已存在
...
// Login 用户登录
func (s *AuthService) Login(ctx context.Context, username, password, ip string) (string, int64, error) {
	// 1. 进行机器人行为检测
...
// CheckBot 检查是否为机器人
func (s *AuthService) CheckBot(ctx context.Context, userID uint64, ip string) bool {
	behavior := algorithm.UserBehavior{
		UserID:    userID,
		IP:        ip,
		Timestamp: time.Now(),
