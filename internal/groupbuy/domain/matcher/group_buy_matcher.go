...
func (m *GroupBuyMatcher) sortByDistance(groups []GroupBuyGroup, lat, lon float64) *GroupBuyGroup {
	slices.SortFunc(groups, func(a, b GroupBuyGroup) int {
		distA := algomath.HaversineDistance(lat, lon, a.Lat, a.Lon)
		distB := algomath.HaversineDistance(lat, lon, b.Lat, b.Lon)
		if distA < distB {
			return -1
		}
		if distA > distB {
...
func (m *GroupBuyMatcher) SmartMatch(
	...
	for i := range available {
		...
		if g.Region == userRegion {
			...
			dist := algomath.HaversineDistance(userLat, userLon, g.Lat, g.Lon)
