...
type ReviewRepository interface {
	...
	// SaveInTx 在事务中保存评论。
	SaveInTx(ctx context.Context, tx any, review *Review) error
	// Get 根据ID获取评论实体。
	Get(ctx context.Context, id uint64) (*Review, error)
	// List 列出指定商品ID的所有评论实体，支持通过状态过滤和分页。
	List(ctx context.Context, productID uint64, status *ReviewStatus, offset, limit int) ([]*Review, int64, error)
	// ListByUser 列出指定用户ID的所有评论实体，支持分页。
	ListByUser(ctx context.Context, userID uint64, offset, limit int) ([]*Review, int64, error)
	// Delete 根据ID删除评论实体。
	Delete(ctx context.Context, id uint64) error
	// DeleteInTx 在事务中删除评论实体。
