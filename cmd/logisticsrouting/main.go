...
const BootstrapName = "logisticsrouting"

// IdempotencyPrefix 幂等性 Redis 键前缀
const IdempotencyPrefix = "logisticsrouting:idem"
...
type AppContext struct {
	Config      *Config
	Routing     *application.LogisticsRoutingService
	Clients     *ServiceClients
	Handler     *routinghttp.Handler
...
func registerGRPC(s *grpc.Server, svc any) {
	ctx := svc.(*AppContext)
	pb.RegisterLogisticsRoutingServiceServer(s, routinggrpc.NewServer(ctx.Routing))
}
...
func initService(cfg any, m *metrics.Metrics) (any, func(), error) {
	...
	routingRepo := persistence.NewLogisticsRoutingRepository(db.RawDB())

	// 5.2 Application (Service)
	query := application.NewLogisticsRoutingQuery(routingRepo)
	manager := application.NewLogisticsRoutingManager(routingRepo, logger.Logger)
	routingService := application.NewLogisticsRoutingService(manager, query)

	// 5.3 Interface (HTTP Handlers)
	handler := routinghttp.NewHandler(routingService, logger.Logger)
