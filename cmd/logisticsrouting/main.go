...
func registerGin(e *gin.Engine, srv any) {
	...
	// 3. 业务路由
	handler := routinghttp.NewHandler(ctx.AppService, slog.Default())
	api := e.Group("/api/v1")
...
func initService(cfg any, m *metrics.Metrics) (any, func(), error) {
	...
	// 4. DDD 分层装配
	bootLog.Info("assembling logistics routing services...")
	repo := persistence.NewLogisticsRoutingRepository(db)
	mgr := application.NewLogisticsRoutingManager(repo, logger.Logger)
