...
func main() {
	...
	grpcServer, grpcErrChan := startGRPCServer(dataWarehouseQueryService, config.Server.GRPC.Addr, config.Server.GRPC.Port)
	if grpcServer == nil {
...
func startGRPCServer(svc *service.DataWarehouseQueryService, addr string, port int) (*grpc.Server, chan error) {
	...
	s := grpc.NewServer()
	v1.RegisterDataWarehouseQueryServiceServer(s, svc)
