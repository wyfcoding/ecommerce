...
import (
	...
	"ecommerce/internal/data_warehouse_query/handler"
...
// Config 定义了 data_warehouse_query-service 的所有配置项。
type Config struct {
	configpkg.Config
	Data struct {
...
const serviceName = "data_warehouse_query-service"
...
func registerGRPC(s *grpc.Server, srv interface{}) {
	v1.RegisterDataWarehouseQueryServer(s, srv.(v1.DataWarehouseQueryServiceServer))
}

// registerGin 将 HTTP 路由注册到 Gin 引擎。
func registerGin(e *gin.Engine, srv interface{}) {
	handler.RegisterRoutes(e, srv.(*service.DataWarehouseQueryService))
}
...
func initService(cfg interface{}, m *metrics.Metrics) (interface{}, func(), error) {
	...
	// 4. 初始化 Repositories
	// data_warehouse_queryRepo := repository.NewDataWarehouseQueryRepo(data)

	// 5. 初始化 Service
	zap.S().Info("initializing data_warehouse_query service...")
	// data_warehouse_queryService := service.NewDataWarehouseQueryService(data_warehouse_queryRepo, config.JWT.Secret, config.JWT.Issuer, config.JWT.Expire)

	// TODO: Replace with actual service initialization
	data_warehouse_queryService := struct{}{
	}
	...
	return data_warehouse_queryService, cleanup, nil
}