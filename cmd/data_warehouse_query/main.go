...
import (
	...
	"ecommerce/internal/data_warehouse_query/data"
	datawarehousequeryhandler "ecommerce/internal/data_warehouse_query/handler"
...
func main() {
	...
	flag.StringVar(&configPath, "conf", "./configs/data_warehouse_query.toml", "config file path")
	flag.Parse()
	...
	// 初始化业务层
	dataWarehouseQueryRepo := data.NewDataWarehouseQueryRepo(dataInstance, redisClient)
	dataWarehouseQueryUsecase := biz.NewDataWarehouseQueryUsecase(dataWarehouseQueryRepo)

	// 初始化服务层
	dataWarehouseQueryService := service.NewDataWarehouseQueryService(dataWarehouseQueryUsecase)

	// 5. 启动 gRPC 和 HTTP Gateway
	grpcServer, grpcErrChan := datawarehousequeryhandler.StartGRPCServer(dataWarehouseQueryService, config.Server.GRPC.Addr, config.Server.GRPC.Port)
	if grpcServer == nil {
		zap.S().Fatalf("failed to start gRPC server: %v", <-grpcErrChan)
	}
	httpServer, httpErrChan := datawarehousequeryhandler.StartHTTPServer(context.Background(), config.Server.Grpc.Addr, config.Server.Grpc.Port, config.Server.HTTP.Addr, config.Server.HTTP.Port)
	if httpServer == nil {
	...
	select {
	case <-quit:
		zap.S().Info("Shutting down data_warehouse_query service...")
	case err := <-grpcErrChan:
		zap.S().Errorf("gRPC server error: %v", err)
		zap.S().Info("Shutting down data_warehouse_query service due to gRPC error...")
	case err := <-httpErrChan:
		zap.S().Errorf("HTTP server error: %v", err)
		zap.S().Info("Shutting down data_warehouse_query service due to HTTP error...")
	}
