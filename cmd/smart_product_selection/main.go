...
func main() {
	...
	flag.StringVar(&configPath, "conf", "./configs/smart_product_selection.toml", "config file path")
	flag.Parse()
	...
	grpcServer, grpcErrChan := startGRPCServer(smartProductSelectionService, config.Server.GRPC.Addr, config.Server.GRPC.Port)
	if grpcServer == nil {
	...
	select {
	case <-quit:
		zap.S().Info("Shutting down smart_product_selection service...")
	case err := <-grpcErrChan:
		zap.S().Errorf("gRPC server error: %v", err)
		zap.S().Info("Shutting down smart_product_selection service due to gRPC error...")
	case err := <-httpErrChan:
		zap.S().Errorf("HTTP server error: %v", err)
		zap.S().Info("Shutting down smart_product_selection service due to HTTP error...")
	}
...
func startGRPCServer(svc *service.SmartProductSelectionService, addr string, port int) (*grpc.Server, chan error) {
	...
	s := grpc.NewServer()
	v1.RegisterSmartProductSelectionServiceServer(s, svc)
...
func startHTTPServer(ctx context.Context, grpcAddr string, grpcPort int, httpAddr string, httpPort int) (*http.Server, chan error) {
	...
	err := v1.RegisterSmartProductSelectionServiceHandlerFromEndpoint(ctx, mux, grpcEndpoint, opts)
	if err != nil {
		errChan <- fmt.Errorf("failed to register gRPC gateway for SmartProductSelectionService: %w", err)
		return nil, errChan
	}
