...
import (
	...
	httpServer "github.com/wyfcoding/ecommerce/internal/file/interfaces/http"
...
type AppContext struct {
	Config     *Config
	AppService *application.FileService
...
func registerGRPC(s *grpc.Server, svc any) {
	ctx := svc.(*AppContext)
	pb.RegisterFileServiceServer(s, grpcServer.NewServer(ctx.AppService))
}
...
func initService(cfg any, m *metrics.Metrics) (any, func(), error) {
	...
	// 4. DDD 分层装配
	bootLog.Info("assembling file domain services...")
	repo := persistence.NewFileRepository(db)
	mgr := application.NewFileManager(repo, logger.Logger)
