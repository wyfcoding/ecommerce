...
func main() {
	...
	// 初始化业务层
	aiModelRepo := data.NewAiModelRepo(dataInstance, redisClient)
	aiModelUsecase := biz.NewAiModelUsecase(aiModelRepo)

	// 初始化服务层
	aiModelService := service.NewAiModelService(aiModelUsecase)

	// 5. 启动 gRPC 和 HTTP Gateway
	grpcServer, grpcErrChan := startGRPCServer(aiModelService, config.Server.GRPC.Addr, config.Server.GRPC.Port)
	if grpcServer == nil {
	...
	case err := <-httpErrChan:
		...
		zap.S().Info("Shutting down ai_model service due to HTTP error...")
	}
...
<<<<<<< HEAD
	...
	// For example:
	// r.POST("/ai_model/predict", handler.Predict)
