...
func main() {
	...
	grpcServer, grpcErrChan := startGRPCServer(realtimeAnalyticsService, config.Server.GRPC.Addr, config.Server.GRPC.Port)
	if grpcServer == nil {
...
func startGRPCServer(svc *service.RealtimeAnalyticsService, addr string, port int) (*grpc.Server, chan error) {
	...
	s := grpc.NewServer()
	v1.RegisterRealtimeAnalyticsServiceServer(s, svc)
