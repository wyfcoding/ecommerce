...
func initService(cfg any, m *metrics.Metrics) (any, func(), error) {
	...
	// 4. DDD 分层装配
	bootLog.Info("assembling notification domain services...")
	repo := persistence.NewNotificationRepository(db)
	notifQuery := application.NewNotificationQuery(repo, logger.Logger)
	notifManager := application.NewNotificationManager(repo, logger.Logger)
