...
func main() {
	...
	grpcServer, grpcErrChan := startGRPCServer(dataProcessingService, config.Server.GRPC.Addr, config.Server.GRPC.Port)
	if grpcServer == nil {
...
func startGRPCServer(svc *service.DataProcessingService, addr string, port int) (*grpc.Server, chan error) {
	...
	s := grpc.NewServer()
	v1.RegisterDataProcessingServiceServer(s, svc)
...
<<<<<<< HEAD
	...
	// For example:
	// r.POST("/data_processing/process", handler.ProcessData)
