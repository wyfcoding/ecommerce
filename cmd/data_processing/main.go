...
import (
	...
	"ecommerce/internal/data_processing/handler"
...
// Config 定义了 data_processing-service 的所有配置项。
type Config struct {
	configpkg.Config
	Data struct {
...
func registerGRPC(s *grpc.Server, srv interface{}) {
	v1.RegisterDataProcessingServer(s, srv.(v1.DataProcessingServiceServer))
}

// registerGin 将 HTTP 路由注册到 Gin 引擎。
func registerGin(e *gin.Engine, srv interface{}) {
	handler.RegisterRoutes(e, srv.(*service.DataProcessingService))
}
...
func initService(cfg interface{}, m *metrics.Metrics) (interface{}, func(), error) {
	...
	// 4. 初始化 Repositories
	// data_processingRepo := repository.NewDataProcessingRepo(data)

	// 5. 初始化 Service
	zap.S().Info("initializing data_processing service...")
	// data_processingService := service.NewDataProcessingService(data_processingRepo, config.JWT.Secret, config.JWT.Issuer, config.JWT.Expire)

	// TODO: Replace with actual service initialization
	data_processingService := struct{}{
	}
	...
	return data_processingService, cleanup, nil
}