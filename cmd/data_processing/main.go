...
import (
	...
	"ecommerce/internal/data_processing/data"
	dataprocessinghandler "ecommerce/internal/data_processing/handler"
...
func main() {
	...
	flag.StringVar(&configPath, "conf", "./configs/data_processing.toml", "config file path")
	flag.Parse()
	...
	// 初始化业务层
	dataProcessingRepo := data.NewDataProcessingRepo(dataInstance, redisClient)
	dataProcessingUsecase := biz.NewDataProcessingUsecase(dataProcessingRepo)

	// 初始化服务层
	dataProcessingService := service.NewDataProcessingService(dataProcessingUsecase)

	// 5. 启动 gRPC 和 HTTP Gateway
	grpcServer, grpcErrChan := dataprocessinghandler.StartGRPCServer(dataProcessingService, config.Server.GRPC.Addr, config.Server.GRPC.Port)
	if grpcServer == nil {
		zap.S().Fatalf("failed to start gRPC server: %v", <-grpcErrChan)
	}
	httpServer, httpErrChan := dataprocessinghandler.StartHTTPServer(context.Background(), config.Server.GRPC.Addr, config.Server.GRPC.Port, config.Server.HTTP.Addr, config.Server.HTTP.Port)
	if httpServer == nil {
	...
	select {
	case <-quit:
		zap.S().Info("Shutting down data_processing service...")
	case err := <-grpcErrChan:
		zap.S().Errorf("gRPC server error: %v", err)
		zap.S().Info("Shutting down data_processing service due to gRPC error...")
	case err := <-httpErrChan:
		zap.S().Errorf("HTTP server error: %v", err)
		zap.S().Info("Shutting down data_processing service due to HTTP error...")
	}
