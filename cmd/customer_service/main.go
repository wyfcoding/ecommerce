...
import (
	...
	"ecommerce/internal/customer_service/data"
	customerservicehandler "ecommerce/internal/customer_service/handler"
...
func main() {
	...
	flag.StringVar(&configPath, "conf", "./configs/customer_service.toml", "config file path")
	flag.Parse()
	...
	// 初始化业务层
	customerServiceRepo := data.NewCustomerServiceRepo(dataInstance, redisClient)
	customerServiceUsecase := biz.NewCustomerServiceUsecase(customerServiceRepo)

	// 初始化服务层
	customerServiceService := service.NewCustomerServiceService(customerServiceUsecase)

	// 5. 启动 gRPC 和 HTTP Gateway
	grpcServer, grpcErrChan := customerservicehandler.StartGRPCServer(customerServiceService, config.Server.GRPC.Addr, config.Server.GRPC.Port)
	if grpcServer == nil {
		zap.S().Fatalf("failed to start gRPC server: %v", <-grpcErrChan)
	}
	httpServer, httpErrChan := customerservicehandler.StartHTTPServer(context.Background(), config.Server.GRPC.Addr, config.Server.GRPC.Port, config.Server.HTTP.Addr, config.Server.HTTP.Port)
	if httpServer == nil {
	...
	select {
	case <-quit:
		zap.S().Info("Shutting down customer_service service...")
	case err := <-grpcErrChan:
		zap.S().Errorf("gRPC server error: %v", err)
		zap.S().Info("Shutting down customer_service service due to gRPC error...")
	case err := <-httpErrChan:
		zap.S().Errorf("HTTP server error: %v", err)
		zap.S().Info("Shutting down customer_service service due to HTTP error...")
	}
