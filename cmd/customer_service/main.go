...
func main() {
	...
	grpcServer, grpcErrChan := startGRPCServer(customerServiceService, config.Server.GRPC.Addr, config.Server.GRPC.Port)
	if grpcServer == nil {
...
func startGRPCServer(svc *service.CustomerServiceService, addr string, port int) (*grpc.Server, chan error) {
	...
	s := grpc.NewServer()
	v1.RegisterCustomerServiceServiceServer(s, svc)
