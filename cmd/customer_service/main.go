...
import (
	...
	"ecommerce/internal/customer_service/handler"
...
// Config 定义了 customer_service-service 的所有配置项。
type Config struct {
	configpkg.Config
	Data struct {
...
func registerGRPC(s *grpc.Server, srv interface{}) {
	v1.RegisterCustomerServiceServer(s, srv.(v1.CustomerServiceServiceServer))
}

// registerGin 将 HTTP 路由注册到 Gin 引擎。
func registerGin(e *gin.Engine, srv interface{}) {
	handler.RegisterRoutes(e, srv.(*service.CustomerServiceService))
}
...
func initService(cfg interface{}, m *metrics.Metrics) (interface{}, func(), error) {
	...
	// 4. 初始化 Repositories
	// customer_serviceRepo := repository.NewCustomerServiceRepo(data)

	// 5. 初始化 Service
	zap.S().Info("initializing customer_service service...")
	// customer_serviceService := service.NewCustomerServiceService(customer_serviceRepo, config.JWT.Secret, config.JWT.Issuer, config.JWT.Expire)

	// TODO: Replace with actual service initialization
	customer_serviceService := struct{}{
	}
	...
	return customer_serviceService, cleanup, nil
}