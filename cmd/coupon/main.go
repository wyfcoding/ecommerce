...
func registerGRPC(s *grpc.Server, svc any) {
	ctx := svc.(*AppContext)
	pb.RegisterCouponServiceServer(s, coupongrpc.NewServer(ctx.AppService))
}
...
func initService(cfg any, m *metrics.Metrics) (any, func(), error) {
	...
	// 4. DDD 分层装配
	bootLog.Info("assembling coupon domain services...")
	repo := persistence.NewCouponRepository(db)
	couponQuery := application.NewCouponQuery(repo)
	couponManager := application.NewCouponManager(repo, logger.Logger)
	service := application.NewCoupon(couponManager, couponQuery)
