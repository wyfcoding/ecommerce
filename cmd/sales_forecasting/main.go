...
func main() {
	...
	grpcServer, grpcErrChan := startGRPCServer(salesForecastingService, config.Server.GRPC.Addr, config.Server.GRPC.Port)
	if grpcServer == nil {
...
func startGRPCServer(svc *service.SalesForecastingService, addr string, port int) (*grpc.Server, chan error) {
	...
	s := grpc.NewServer()
	v1.RegisterSalesForecastingServiceServer(s, svc)
