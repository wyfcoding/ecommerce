...
func main() {
	...
	flag.StringVar(&configPath, "conf", "./configs/auth.toml", "config file path")
	flag.Parse()
	...
	// 初始化业务层
	authRepo := data.NewAuthRepo(dataInstance, redisClient)
	...
	grpcServer, grpcErrChan := authhandler.StartGRPCServer(authService, config.Server.GRPC.Addr, config.Server.GRPC.Port)
	if grpcServer == nil {
		zap.S().Fatalf("failed to start gRPC server: %v", <-grpcErrChan)
	}
	httpServer, httpErrChan := authhandler.StartHTTPServer(context.Background(), config.Server.GRPC.Addr, config.Server.GRPC.Port, config.Server.HTTP.Addr, config.Server.HTTP.Port)
	if httpServer == nil {
	...
	select {
	case <-quit:
		zap.S().Info("Shutting down auth service...")
	case err := <-grpcErrChan:
		zap.S().Errorf("gRPC server error: %v", err)
		zap.S().Info("Shutting down auth service due to gRPC error...")
	case err := <-httpErrChan:
		zap.S().Errorf("HTTP server error: %v", err)
		zap.S().Info("Shutting down auth service due to HTTP error...")
	}
