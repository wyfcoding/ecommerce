...
func main() {
	...
	grpcServer, grpcErrChan := startGRPCServer(authService, config.Server.GRPC.Addr, config.Server.GRPC.Port)
	if grpcServer == nil {
...
func startHTTPServer(ctx context.Context, grpcAddr string, grpcPort int, httpAddr string, httpPort int) (*http.Server, chan error) {
	...
	err := v1.RegisterAuthServiceHandlerFromEndpoint(ctx, mux, grpcEndpoint, opts)
	if err != nil {
		errChan <- fmt.Errorf("failed to register gRPC gateway for AuthService: %w", err)
		return nil, errChan
	}
