...
import (
	...
	"ecommerce/internal/config/data"
	confighandler "ecommerce/internal/config/handler"
...
func main() {
	...
	flag.StringVar(&configPath, "conf", "./configs/config.toml", "config file path")
	flag.Parse()
	...
	// 初始化业务层
	configRepo := data.NewConfigRepo(dataInstance, redisClient)
	...
	grpcServer, grpcErrChan := confighandler.StartGRPCServer(configService, config.Server.GRPC.Addr, config.Server.GRPC.Port)
	if grpcServer == nil {
		zap.S().Fatalf("failed to start gRPC server: %v", <-grpcErrChan)
	}
	httpServer, httpErrChan := confighandler.StartHTTPServer(context.Background(), config.Server.GRPC.Addr, config.Server.GRPC.Port, config.Server.HTTP.Addr, config.Server.HTTP.Port)
	if httpServer == nil {
	...
	select {
	case <-quit:
		zap.S().Info("Shutting down config service...")
	case err := <-grpcErrChan:
		zap.S().Errorf("gRPC server error: %v", err)
		zap.S().Info("Shutting down config service due to gRPC error...")
	case err := <-httpErrChan:
		zap.S().Errorf("HTTP server error: %v", err)
		zap.S().Info("Shutting down config service due to HTTP error...")
	}
