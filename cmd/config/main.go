...
func main() {
	...
	grpcServer, grpcErrChan := startGRPCServer(configService, config.Server.GRPC.Addr, config.Server.GRPC.Port)
	if grpcServer == nil {
...
func startGRPCServer(svc *service.ConfigService, addr string, port int) (*grpc.Server, chan error) {
	...
	s := grpc.NewServer()
	v1.RegisterConfigServiceServer(s, svc)
...
func startHTTPServer(ctx context.Context, grpcAddr string, grpcPort int, httpAddr string, httpPort int) (*http.Server, chan error) {
	...
	// For example:
	// r.GET("/config/settings", handler.GetSettings)
