...
import (
	...
	flashsalesearch "github.com/wyfcoding/ecommerce/internal/flashsale/infrastructure/persistence/elasticsearch"
	flashsalemysql "github.com/wyfcoding/ecommerce/internal/flashsale/infrastructure/persistence/mysql"
	flashsaleredis "github.com/wyfcoding/ecommerce/internal/flashsale/infrastructure/persistence/redis"
	flashsaleconsumer "github.com/wyfcoding/ecommerce/internal/flashsale/interfaces/consumer"
...
type Config struct {
	...
	Search           struct {
		FlashsaleIndex string `mapstructure:"flashsale_index" toml:"flashsale_index"`
...
func registerGRPC(s *grpc.Server, ctx *AppContext) {
	pb.RegisterFlashSaleServiceServer(s, flashsalegrpc.NewServer(ctx.Cmd, ctx.Query))
}
...
func initService(cfg *Config, m *metrics.Metrics) (*AppContext, func(), error) {
	...
	// 5.1 Infrastructure (Persistence & Cache)
	flashsaleRepo := flashsalemysql.NewFlashSaleRepository(db.RawDB())
	flashsaleReadRepo := flashsaleredis.NewFlashsaleReadRepository(redisCache.GetClient(), c.Cache.DefaultExpiration)
	orderReadRepo := flashsaleredis.NewFlashsaleOrderReadRepository(redisCache.GetClient(), c.Cache.DefaultExpiration)
	flashsaleSearchRepo := flashsalesearch.NewFlashsaleSearchRepository(esClient, c.Search.FlashsaleIndex)
	orderSearchRepo := flashsalesearch.NewFlashsaleOrderSearchRepository(esClient, c.Search.OrderIndex)
	...
	querySvc := application.NewFlashSaleQueryService(flashsaleRepo, flashsaleReadRepo, orderReadRepo, flashsaleSearchRepo, orderSearchRepo, logger.Logger)

	// 5.3 Projection Consumers (Flashsale Events -> Read Model)
	projectionService := application.NewFlashSaleProjectionService(flashsaleRepo, flashsaleReadRepo, orderReadRepo, flashsaleSearchRepo, orderSearchRepo, logger.Logger)
	projectionHandler := flashsaleconsumer.NewFlashSaleProjectionHandler(projectionService, logger.Logger)
	projectionTopics := []string{
		domain.FlashsaleCreatedEventType,
		domain.FlashsaleOrderCreatedEventType,
		domain.FlashsaleOrderCancelledEventType,
		domain.FlashsaleOrderPaidEventType,
	}
	...
	handler := flashsalehttp.NewHandler(commandSvc, querySvc, logger.Logger)
