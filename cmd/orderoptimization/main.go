...
import (
	...
	optigrpc "github.com/wyfcoding/ecommerce/internal/orderoptimization/interfaces/grpc"
	optihttp "github.com/wyfcoding/ecommerce/internal/orderoptimization/interfaces/http"
...
func registerGin(e *gin.Engine, srv any) {
	...
	// 3. 业务路由
	handler := optihttp.NewHandler(ctx.AppService, slog.Default())
	api := e.Group("/api/v1")
...
func initService(cfg any, m *metrics.Metrics) (any, func(), error) {
	...
	// 4. DDD 分层装配
	bootLog.Info("assembling order optimization services...")
	repo := persistence.NewOrderOptimizationRepository(db)
	mgr := application.NewOptimizationManager(repo, logger.Logger)
