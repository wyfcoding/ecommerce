...
func registerGRPC(s *grpc.Server, svc any) {
	ctx := svc.(*AppContext)
	pb.RegisterInventoryForecastServiceServer(s, forecastgrpc.NewServer(ctx.AppService))
}
...
func registerGin(e *gin.Engine, srv any) {
	...
	// 3. 业务路由
	handler := forecasthttp.NewHandler(ctx.AppService, slog.Default())
	api := e.Group("/api/v1")
...
func initService(cfg any, m *metrics.Metrics) (any, func(), error) {
	...
	// 4. DDD 分层装配
	bootLog.Info("assembling inventory forecast services...")
	repo := persistence.NewInventoryForecastRepository(db)
	mgr := application.NewInventoryForecastManager(repo, logger.Logger)
