...
import (
	...
	"github.com/wyfcoding/ecommerce/internal/usertier/infrastructure/messaging"
...
const BootstrapName = "usertier"

// IdempotencyPrefix 幂等性 Redis 键前缀
const IdempotencyPrefix = "usertier:idem"
...
type AppContext struct {
	Config      *Config
	UserTier    *application.UserTierService
	Clients     *ServiceClients
	Handler     *usertierhttp.Handler
...
func registerGRPC(s *grpc.Server, ctx *AppContext) {
	pb.RegisterUserTierServiceServer(s, usertiergrpc.NewServer(ctx.UserTier))
}
...
func initService(cfg *Config, m *metrics.Metrics) (*AppContext, func(), error) {
	...
	usertierRepo := persistence.NewUserTierRepository(db.RawDB())

	// 6.2 Application (Service)
	commandSvc := application.NewUserTierCommandService(usertierRepo, outboxPublisher, logger.Logger)
	querySvc := application.NewUserTierQueryService(usertierRepo, commandSvc)
	usertierService := application.NewUserTierService(usertierRepo, commandSvc, querySvc)

	// 5.3 Interface (HTTP Handlers)
	handler := usertierhttp.NewHandler(usertierService, logger.Logger)
