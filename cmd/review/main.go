...
import (
	...
	"github.com/wyfcoding/ecommerce/internal/review/infrastructure/messaging"
...
func registerGRPC(s *grpc.Server, ctx *AppContext) {
	pb.RegisterReviewServiceServer(s, reviewgrpc.NewServer(ctx.Review))
}
...
func initService(cfg *Config, m *metrics.Metrics) (*AppContext, func(), error) {
	...
	reviewRepo := persistence.NewReviewRepository(db.RawDB(), logger.Logger)

	// 6.2 Application (Service)
	querySvc := application.NewReviewQueryService(reviewRepo, logger.Logger)
	commandSvc := application.NewReviewCommandService(reviewRepo, outboxPublisher, logger.Logger)
	reviewService := application.NewReview(commandSvc, querySvc, logger.Logger)

	// 5.3 Interface (HTTP Handlers)
	handler := reviewhttp.NewHandler(reviewService, logger.Logger)
