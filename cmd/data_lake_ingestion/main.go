...
import (
	...
	"ecommerce/internal/data_lake_ingestion/data"
	datalakeingestionhandler "ecommerce/internal/data_lake_ingestion/handler"
...
func main() {
	...
	flag.StringVar(&configPath, "conf", "./configs/data_lake_ingestion.toml", "config file path")
	flag.Parse()
	...
	// 初始化业务层
	dataLakeIngestionRepo := data.NewDataLakeIngestionRepo(dataInstance, redisClient)
	dataLakeIngestionUsecase := biz.NewDataLakeIngestionUsecase(dataLakeIngestionRepo)

	// 初始化服务层
	dataLakeIngestionService := service.NewDataLakeIngestionService(dataLakeIngestionUsecase)

	// 5. 启动 gRPC 和 HTTP Gateway
	grpcServer, grpcErrChan := datalakeingestionhandler.StartGRPCServer(dataLakeIngestionService, config.Server.GRPC.Addr, config.Server.GRPC.Port)
	if grpcServer == nil {
		zap.S().Fatalf("failed to start gRPC server: %v", <-grpcErrChan)
	}
	httpServer, httpErrChan := datalakeingestionhandler.StartHTTPServer(context.Background(), config.Server.GRPC.Addr, config.Server.Grpc.Port, config.Server.HTTP.Addr, config.Server.HTTP.Port)
	if httpServer == nil {
	...
	select {
	case <-quit:
		zap.S().Info("Shutting down data_lake_ingestion service...")
	case err := <-grpcErrChan:
		zap.S().Errorf("gRPC server error: %v", err)
		zap.S().Info("Shutting down data_lake_ingestion service due to gRPC error...")
	case err := <-httpErrChan:
		zap.S().Errorf("HTTP server error: %v", err)
		zap.S().Info("Shutting down data_lake_ingestion service due to HTTP error...")
	}
