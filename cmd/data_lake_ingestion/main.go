...
func main() {
	...
	grpcServer, grpcErrChan := startGRPCServer(dataLakeIngestionService, config.Server.GRPC.Addr, config.Server.GRPC.Port)
	if grpcServer == nil {
...
func startHTTPServer(ctx context.Context, grpcAddr string, grpcPort int, httpAddr string, httpPort int) (*http.Server, chan error) {
	...
	// For example:
	// r.POST("/data_lake_ingestion/data", handler.IngestData)
