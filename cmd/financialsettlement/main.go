...
import (
	...
	grpcServer "github.com/wyfcoding/ecommerce/internal/financialsettlement/interfaces/grpc"
	httpServer "github.com/wyfcoding/ecommerce/internal/financialsettlement/interfaces/http"
...
// BootstrapName 服务标识。
const BootstrapName = "financialsettlement"
...
func registerGRPC(s *grpc.Server, svc any) {
	ctx := svc.(*AppContext)
	pb.RegisterFinancialSettlementServer(s, grpcServer.NewServer(ctx.AppService))
}
...
func initService(cfg any, m *metrics.Metrics) (any, func(), error) {
	...
	// 4. DDD 分层装配
	bootLog.Info("assembling financial settlement domain services...")
	repo := persistence.NewSettlementRepository(db)
	manager := application.NewSettlementManager(repo, logger.Logger)
