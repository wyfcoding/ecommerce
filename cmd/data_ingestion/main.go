...
func main() {
	...
	grpcServer, grpcErrChan := startGRPCServer(dataIngestionService, config.Server.GRPC.Addr, config.Server.GRPC.Port)
	if grpcServer == nil {
...
func startGRPCServer(svc *service.DataIngestionService, addr string, port int) (*grpc.Server, chan error) {
	...
	s := grpc.NewServer()
	v1.RegisterDataIngestionServiceServer(s, svc)
