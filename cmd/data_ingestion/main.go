...
import (
	...
	"ecommerce/internal/data_ingestion/data"
	dataingestionhandler "ecommerce/internal/data_ingestion/handler"
...
func main() {
	...
	flag.StringVar(&configPath, "conf", "./configs/data_ingestion.toml", "config file path")
	flag.Parse()
	...
	// 初始化业务层
	dataIngestionRepo := data.NewDataIngestionRepo(dataInstance, redisClient)
	dataIngestionUsecase := biz.NewDataIngestionUsecase(dataIngestionRepo)

	// 初始化服务层
	dataIngestionService := service.NewDataIngestionService(dataIngestionUsecase)

	// 5. 启动 gRPC 和 HTTP Gateway
	grpcServer, grpcErrChan := dataingestionhandler.StartGRPCServer(dataIngestionService, config.Server.GRPC.Addr, config.Server.GRPC.Port)
	if grpcServer == nil {
		zap.S().Fatalf("failed to start gRPC server: %v", <-grpcErrChan)
	}
	httpServer, httpErrChan := dataingestionhandler.StartHTTPServer(context.Background(), config.Server.GRPC.Addr, config.Server.GRPC.Port, config.Server.HTTP.Addr, config.Server.HTTP.Port)
	if httpServer == nil {
	...
	select {
	case <-quit:
		zap.S().Info("Shutting down data_ingestion service...")
	case err := <-grpcErrChan:
		zap.S().Errorf("gRPC server error: %v", err)
		zap.S().Info("Shutting down data_ingestion service due to gRPC error...")
	case err := <-httpErrChan:
		zap.S().Errorf("HTTP server error: %v", err)
		zap.S().Info("Shutting down data_ingestion service due to HTTP error...")
	}
