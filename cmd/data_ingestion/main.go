...
import (
	...
	"ecommerce/internal/data_ingestion/handler"
...
// Config 定义了 data_ingestion-service 的所有配置项。
type Config struct {
	configpkg.Config
	Data struct {
...
func registerGRPC(s *grpc.Server, srv interface{}) {
	v1.RegisterDataIngestionServer(s, srv.(v1.DataIngestionServiceServer))
}

// registerGin 将 HTTP 路由注册到 Gin 引擎。
func registerGin(e *gin.Engine, srv interface{}) {
	handler.RegisterRoutes(e, srv.(*service.DataIngestionService))
}
...
func initService(cfg interface{}, m *metrics.Metrics) (interface{}, func(), error) {
	...
	// 4. 初始化 Repositories
	// data_ingestionRepo := repository.NewDataIngestionRepo(data)

	// 5. 初始化 Service
	zap.S().Info("initializing data_ingestion service...")
	// data_ingestionService := service.NewDataIngestionService(data_ingestionRepo, config.JWT.Secret, config.JWT.Issuer, config.JWT.Expire)

	// TODO: Replace with actual service initialization
	data_ingestionService := struct{}{
	}
	...
	return data_ingestionService, cleanup, nil
}