...
import (
	...
	recommendationmysql "github.com/wyfcoding/ecommerce/internal/recommendation/infrastructure/persistence/mysql"
	recommendationredis "github.com/wyfcoding/ecommerce/internal/recommendation/infrastructure/persistence/redis"
	consumer "github.com/wyfcoding/ecommerce/internal/recommendation/interfaces/consumer"
...
const IdempotencyPrefix = "recommendation:idem"

// Config 服务扩展配置
type Config struct {
	configpkg.Config `mapstructure:",squash"`
	Search           struct {
		RecommendationIndex string `mapstructure:"recommendation_index" toml:"recommendation_index"`
...
type AppContext struct {
	...
	Handler     *recommendationhttp.Handler
...
func registerGRPC(s *grpc.Server, ctx *AppContext) {
	pb.RegisterRecommendationServiceServer(s, recommendationgrpc.NewServer(ctx.Cmd, ctx.Query, logging.Default().Logger))
}
...
func initService(cfg *Config, m *metrics.Metrics) (*AppContext, func(), error) {
	...
	recommendationRepo := recommendationmysql.NewRecommendationRepository(db.RawDB())
	recommendationReadRepo := recommendationredis.NewRecommendationReadRepository(redisCache.GetClient(), c.Cache.DefaultExpiration)
	recommendationSearchRepo := recommendationsearch.NewRecommendationSearchRepository(esClient, c.Search.RecommendationIndex)

	// 6.2 Application (Service)
	querySvc := application.NewRecommendationQueryService(recommendationRepo, recommendationReadRepo, recommendationSearchRepo)
	commandSvc := application.NewRecommendationCommandService(recommendationRepo, outbox.NewPublisher(outboxMgr), logger.Logger)

	// 5.3 Projection Consumers (Recommendation Events -> Read Model)
	projectionService := application.NewRecommendationProjectionService(recommendationRepo, recommendationReadRepo, recommendationSearchRepo, logger.Logger)
	projectionHandler := consumer.NewRecommendationProjectionHandler(projectionService, logger.Logger)
	projectionTopics := []string{
		domain.RecommendationChangedEventType,
		domain.RecommendationDeletedEventType,
	}
	...
	handler := recommendationhttp.NewHandler(commandSvc, querySvc, logger.Logger)
