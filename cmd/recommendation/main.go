...
func registerGRPC(s *grpc.Server, ctx *AppContext) {
	pb.RegisterRecommendationServiceServer(s, recommendationgrpc.NewServer(ctx.Recommendation, logging.Default().Logger))
}
...
func initService(cfg *Config, m *metrics.Metrics) (*AppContext, func(), error) {
	...
	recommendationRepo := persistence.NewRecommendationRepository(db.RawDB())

	// 6.2 Application (Service)
	querySvc := application.NewRecommendationQueryService(recommendationRepo)
	commandSvc := application.NewRecommendationCommandService(recommendationRepo, outboxPublisher, logger.Logger)
	recommendationService := application.NewRecommendationService(commandSvc, querySvc, logger.Logger)

	// 5.3 Interface (HTTP Handlers)
	handler := recommendationhttp.NewHandler(recommendationService, logger.Logger)
