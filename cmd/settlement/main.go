...
import (
	...
	"ecommerce/internal/settlement/data"
...
func main() {
	...
	flag.StringVar(&configPath, "conf", "./configs/settlement.toml", "config file path")
	flag.Parse()
	...
	// 初始化业务层
	settlementRepo := data.NewSettlementRepo(dataInstance, redisClient)
	...
	grpcServer, grpcErrChan := startGRPCServer(settlementService, config.Server.GRPC.Addr, config.Server.GRPC.Port)
	if grpcServer == nil {
	...
	select {
	case <-quit:
		zap.S().Info("Shutting down settlement service...")
	case err := <-grpcErrChan:
		zap.S().Errorf("gRPC server error: %v", err)
		zap.S().Info("Shutting down settlement service due to gRPC error...")
	case err := <-httpErrChan:
		zap.S().Errorf("HTTP server error: %v", err)
		zap.S().Info("Shutting down settlement service due to HTTP error...")
	}
...
func startGRPCServer(svc *service.SettlementService, addr string, port int) (*grpc.Server, chan error) {
	...
	s := grpc.NewServer()
	v1.RegisterSettlementServiceServer(s, svc)
...
func startHTTPServer(ctx context.Context, grpcAddr string, grpcPort int, httpAddr string, httpPort int) (*http.Server, chan error) {
	...
	err := v1.RegisterSettlementServiceHandlerFromEndpoint(ctx, mux, grpcEndpoint, opts)
	if err != nil {
		errChan <- fmt.Errorf("failed to register gRPC gateway for SettlementService: %w", err)
		return nil, errChan
	}
