...
import (
	...
	"ecommerce/internal/logistics/data"
	logisticshandler "ecommerce/internal/logistics/handler"
...
func main() {
	...
	flag.StringVar(&configPath, "conf", "./configs/logistics.toml", "config file path")
	flag.Parse()
	...
	// 初始化业务层
	logisticsRepo := data.NewLogisticsRepo(dataInstance, redisClient)
	logisticsUsecase := biz.NewLogisticsUsecase(logisticsRepo)

	// 初始化服务层
	logisticsService := service.NewLogisticsService(logisticsUsecase)

	// 5. 启动 gRPC 和 HTTP Gateway
	grpcServer, grpcErrChan := logisticshandler.StartGRPCServer(logisticsService, config.Server.GRPC.Addr, config.Server.GRPC.Port)
	if grpcServer == nil {
		zap.S().Fatalf("failed to start gRPC server: %v", <-grpcErrChan)
	}
	httpServer, httpErrChan := logisticshandler.StartHTTPServer(context.Background(), config.Server.GRPC.Addr, config.Server.GRPC.Port, config.Server.HTTP.Addr, config.Server.HTTP.Port)
	if httpServer == nil {
	...
	select {
	case <-quit:
		zap.S().Info("Shutting down logistics service...")
	case err := <-grpcErrChan:
		zap.S().Errorf("gRPC server error: %v", err)
		zap.S().Info("Shutting down logistics service due to gRPC error...")
	case err := <-httpErrChan:
		zap.S().Errorf("HTTP server error: %v", err)
		zap.S().Info("Shutting down logistics service due to HTTP error...")
	}
