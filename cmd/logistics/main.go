...
func main() {
	...
	grpcServer, grpcErrChan := startGRPCServer(logisticsService, config.Server.GRPC.Addr, config.Server.GRPC.Port)
	if grpcServer == nil {
...
func startGRPCServer(svc *service.LogisticsService, addr string, port int) (*grpc.Server, chan error) {
	...
	s := grpc.NewServer()
	v1.RegisterLogisticsServiceServer(s, svc)
