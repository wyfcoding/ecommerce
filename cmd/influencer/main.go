...
import (
	...
	"github.com/wyfcoding/ecommerce/internal/influencer/infrastructure"
	"github.com/wyfcoding/ecommerce/internal/influencer/interfaces"
...
func main() {
	// 1. 加载配置
	cfg := &config.Config{}
	if err := config.Load("configs/influencer/config.toml", cfg); err != nil {
	...
	// 5. 自动迁移
	if err := db.DB.AutoMigrate(&domain.Influencer{}, &domain.Campaign{}); err != nil {
		logger.Error("failed to migrate database", "error", err)
		os.Exit(1)
	}

	// 6. 依赖注入
	repo := infrastructure.NewGormInfluencerRepository(db.DB)
	app := application.NewInfluencerService(repo)
	handler := interfaces.NewInfluencerHandler(app)
	...
	s := grpc.NewServer()
	pb.RegisterInfluencerServiceServer(s, handler)
