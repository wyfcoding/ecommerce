...
func main() {
	...
	grpcServer, grpcErrChan := startGRPCServer(inventoryService, config.Server.GRPC.Addr, config.Server.GRPC.Port)
	if grpcServer == nil {
...
func startGRPCServer(svc *service.InventoryService, addr string, port int) (*grpc.Server, chan error) {
	...
	s := grpc.NewServer()
	v1.RegisterInventoryServiceServer(s, svc)
...
<<<<<<< HEAD
	...
	// For example:
	// r.GET("/inventory/stock/:sku_id", handler.GetStock)
