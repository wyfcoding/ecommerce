...
import (
	...
	grpcServer "github.com/wyfcoding/ecommerce/internal/advancedcoupon/interfaces/grpc"
	httpServer "github.com/wyfcoding/ecommerce/internal/advancedcoupon/interfaces/http"
...
type AppContext struct {
	Config     *Config
	AppService *application.AdvancedCoupon
...
func registerGRPC(s *grpc.Server, svc any) {
	ctx := svc.(*AppContext)
	pb.RegisterAdvancedCouponServiceServer(s, grpcServer.NewServer(ctx.AppService))
}
...
func initService(cfg any, m *metrics.Metrics) (any, func(), error) {
	...
	// 4. DDD 分层装配
	bootLog.Info("assembling advanced coupon services...")
	repo := persistence.NewAdvancedCouponRepository(db)
	service := application.NewAdvancedCoupon(repo, logger.Logger)
