...
import (
	...
	subscriptionhandler "ecommerce/internal/subscription/handler"
...
func main() {
	...
	flag.StringVar(&configPath, "conf", "./configs/subscription.toml", "config file path")
	flag.Parse()
	...
	// 7. 启动 gRPC 服务器
	grpcServer, grpcErrChan := subscriptionhandler.StartGRPCServer(subscriptionService, cfg.Server.GRPC.Addr, cfg.Server.GRPC.Port)
	if grpcServer == nil {
		zap.S().Fatalf("failed to start gRPC server: %v", <-grpcErrChan)
	}

	// 8. 启动 Gin HTTP 服务器
	ginServer, ginErrChan := subscriptionhandler.StartHTTPServer(subscriptionService, cfg.Server.HTTP.Addr, cfg.Server.HTTP.Port)
	if ginServer == nil {
	...
	select {
	case <-quit:
		zap.S().Info("Shutting down subscription service...")
