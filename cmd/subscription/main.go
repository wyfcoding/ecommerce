...
func registerGRPC(s *grpc.Server, srv any) {
	ctx := srv.(*AppContext)
	pb.RegisterSubscriptionServiceServer(s, subscriptiongrpc.NewServer(ctx.AppService))
}
...
func registerGin(e *gin.Engine, srv any) {
	...
	// 3. 业务路由
	handler := subscriptionhttp.NewHandler(ctx.AppService, slog.Default())
	api := e.Group("/api/v1")
...
func initService(cfg any, m *metrics.Metrics) (any, func(), error) {
	...
	// 4. DDD 分层装配
	bootLog.Info("assembling subscription domain services...")
	repo := persistence.NewSubscriptionRepository(db)
	mgr := application.NewSubscriptionManager(repo, logger.Logger)
