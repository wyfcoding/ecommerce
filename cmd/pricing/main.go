...
func main() {
	...
	grpcServer, grpcErrChan := startGRPCServer(pricingService, config.Server.GRPC.Addr, config.Server.GRPC.Port)
	if grpcServer == nil {
...
func startGRPCServer(svc *service.PricingService, addr string, port int) (*grpc.Server, chan error) {
	...
	s := grpc.NewServer()
	v1.RegisterPricingServiceServer(s, svc)
