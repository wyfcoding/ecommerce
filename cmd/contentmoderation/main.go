...
import (
	...
	grpcServer "github.com/wyfcoding/ecommerce/internal/contentmoderation/interfaces/grpc"
	httpServer "github.com/wyfcoding/ecommerce/internal/contentmoderation/interfaces/http"
...
// BootstrapName 服务标识。
const BootstrapName = "contentmoderation"
...
type AppContext struct {
	Config     *Config
	AppService *application.ModerationService
...
func registerGRPC(s *grpc.Server, svc any) {
	ctx := svc.(*AppContext)
	pb.RegisterContentModerationServiceServer(s, grpcServer.NewServer(ctx.AppService))
}
...
func initService(cfg any, m *metrics.Metrics) (any, func(), error) {
	...
	// 4. DDD 分层装配
	bootLog.Info("assembling content moderation services...")
	repo := persistence.NewModerationRepository(db)
	mgr := application.NewModerationManager(repo, logger.Logger)
	query := application.NewModerationQuery(repo)
	service := application.NewModerationService(mgr, query)
