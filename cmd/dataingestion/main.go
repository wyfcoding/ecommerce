...
import (
	...
	"github.com/wyfcoding/ecommerce/internal/dataingestion/infrastructure/messaging"
	...
	ingestionhttp "github.com/wyfcoding/ecommerce/internal/dataingestion/interfaces/http"
...
const IdempotencyPrefix = "dataingestion:idem"
...
type AppContext struct {
	Config      *Config
	Ingestion   *application.DataIngestionService
	Clients     *ServiceClients
	Handler     *ingestionhttp.Handler
...
func registerGRPC(s *grpc.Server, ctx *AppContext) {
	pb.RegisterDataIngestionServiceServer(s, ingestiongrpc.NewServer(ctx.Ingestion))
}
...
func initService(cfg *Config, m *metrics.Metrics) (*AppContext, func(), error) {
	...
	ingestionRepo := persistence.NewDataIngestionRepository(db.RawDB())

	// 6.2 Application (Service)
	querySvc := application.NewDataIngestionQueryService(ingestionRepo)
	commandSvc := application.NewDataIngestionCommandService(ingestionRepo, outboxPublisher, logger.Logger)
	ingestionService := application.NewDataIngestionService(commandSvc, querySvc)

	// 5.3 Interface (HTTP Handlers)
	handler := ingestionhttp.NewHandler(ingestionService, logger.Logger)
