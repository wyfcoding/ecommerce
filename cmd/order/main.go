...
import (
	...
	"ecommerce/internal/order/config"
...
func init() {
	// 注册命令行参数。
	flag.StringVar(&configPath, "conf", "configs/order.toml", "config path, eg: -conf configs/order.toml")
}
...
func main() {
	...
	zap.S().Infof("order service starting with config: %+v", conf)
	...
	orderService := service.NewOrderService(
		...
		conf.Business.OrderExpirationMinutes,
	)
	...
	go func() {
		...
		err = v1.RegisterOrderHandler(ctx, gwmux, conn)
		if err != nil {
