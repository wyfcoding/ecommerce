...
func registerGRPC(s *grpc.Server, svc any) {
	ctx := svc.(*AppContext)
	pb.RegisterAftersalesServer(s, grpcServer.NewServer(ctx.AppService))
}
...
func initService(cfg any, m *metrics.Metrics) (any, func(), error) {
	...
	// 5. DDD 分层装配
	bootLog.Info("assembling aftersales domain services...")
	repo := persistence.NewAfterSalesRepository(db)
	service := application.NewAfterSalesService(repo, idGen, logger.Logger)
