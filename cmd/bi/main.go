...
import (
	...
	"ecommerce/internal/bi/data"
	bihandler "ecommerce/internal/bi/handler"
...
func main() {
	...
	flag.StringVar(&configPath, "conf", "./configs/bi.toml", "config file path")
	flag.Parse()
	...
	// 初始化业务层
	biRepo := data.NewBiRepo(dataInstance, redisClient)
	biUsecase := biz.NewBiUsecase(biRepo)

	// 初始化服务层
	biService := service.NewBiService(biUsecase)

	// 5. 启动 gRPC 和 HTTP Gateway
	grpcServer, grpcErrChan := bihandler.StartGRPCServer(biService, config.Server.GRPC.Addr, config.Server.GRPC.Port)
	if grpcServer == nil {
		zap.S().Fatalf("failed to start gRPC server: %v", <-grpcErrChan)
	}
	httpServer, httpErrChan := bihandler.StartHTTPServer(context.Background(), config.Server.GRPC.Addr, config.Server.GRPC.Port, config.Server.HTTP.Addr, config.Server.HTTP.Port)
	if httpServer == nil {
	...
	select {
	case <-quit:
		zap.S().Info("Shutting down bi service...")
	case err := <-grpcErrChan:
		zap.S().Errorf("gRPC server error: %v", err)
		zap.S().Info("Shutting down bi service due to gRPC error...")
	case err := <-httpErrChan:
		zap.S().Errorf("HTTP server error: %v", err)
		zap.S().Info("Shutting down bi service due to HTTP error...")
	}
