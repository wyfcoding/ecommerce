...
func registerGRPC(s *grpc.Server, srv interface{}) {
	v1.RegisterBIServer(s, srv.(v1.BIServiceServer))
}

// registerGin 将 HTTP 路由注册到 Gin 引擎。
func registerGin(e *gin.Engine, srv interface{}) {
	handler.RegisterRoutes(e, srv.(*service.BIService))
}
...
func initService(cfg interface{}, m *metrics.Metrics) (interface{}, func(), error) {
	...
	// 4. 初始化 Repositories
	// biRepo := repository.NewBIRepo(data)

	// 5. 初始化 Service
	zap.S().Info("initializing bi service...")
	// biService := service.NewBIService(biRepo, config.JWT.Secret, config.JWT.Issuer, config.JWT.Expire)
