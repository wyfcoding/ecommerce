...
import (
	...
	grpcServer "github.com/wyfcoding/ecommerce/internal/dynamicpricing/interfaces/grpc"
	httpServer "github.com/wyfcoding/ecommerce/internal/dynamicpricing/interfaces/http"
...
func initService(cfg any, m *metrics.Metrics) (any, func(), error) {
	...
	// 4. DDD 分层装配
	bootLog.Info("assembling dynamic pricing services...")
	repo := persistence.NewPricingRepository(db)
	mgr := application.NewDynamicPricingManager(repo, logger.Logger)
	query := application.NewDynamicPricingQuery(repo)
	service := application.NewDynamicPricingService(mgr, query)
