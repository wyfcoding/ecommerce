...
import (
	...
	"ecommerce/internal/payment/data"
...
func main() {
	...
	flag.StringVar(&configPath, "conf", "./configs/payment.toml", "config file path")
	flag.Parse()
	...
	// 初始化业务层
	paymentRepo := data.NewPaymentRepo(dataInstance, redisClient)
	...
	grpcServer, grpcErrChan := startGRPCServer(paymentService, config.Server.GRPC.Addr, config.Server.GRPC.Port)
	if grpcServer == nil {
	...
	select {
	case <-quit:
		zap.S().Info("Shutting down payment service...")
	case err := <-grpcErrChan:
		zap.S().Errorf("gRPC server error: %v", err)
		zap.S().Info("Shutting down payment service due to gRPC error...")
	case err := <-httpErrChan:
		zap.S().Errorf("HTTP server error: %v", err)
		zap.S().Info("Shutting down payment service due to HTTP error...")
	}
...
func startGRPCServer(svc *service.PaymentService, addr string, port int) (*grpc.Server, chan error) {
	...
	s := grpc.NewServer()
	v1.RegisterPaymentServiceServer(s, svc)
...
func startHTTPServer(ctx context.Context, grpcAddr string, grpcPort int, httpAddr string, httpPort int) (*http.Server, chan error) {
	...
	err := v1.RegisterPaymentServiceHandlerFromEndpoint(ctx, mux, grpcEndpoint, opts)
	if err != nil {
		errChan <- fmt.Errorf("failed to register gRPC gateway for PaymentService: %w", err)
		return nil, errChan
	}
