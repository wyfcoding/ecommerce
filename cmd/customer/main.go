...
import (
	...
	grpcServer "github.com/wyfcoding/ecommerce/internal/customer/interfaces/grpc"
	httpServer "github.com/wyfcoding/ecommerce/internal/customer/interfaces/http"
...
type AppContext struct {
	Config     *Config
	AppService *application.Customer
...
func initService(cfg any, m *metrics.Metrics) (any, func(), error) {
	...
	// 4. DDD 分层装配
	bootLog.Info("assembling customer domain services...")
	repo := persistence.NewCustomerRepository(db)
	mgr := application.NewCustomerManager(repo, logger.Logger)
	query := application.NewCustomerQuery(repo)
	service := application.NewCustomer(mgr, query)
