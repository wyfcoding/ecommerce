...
type AppContext struct {
	Config     *Config
	AppService *application.Wishlist
...
func registerGRPC(s *grpc.Server, svc any) {
	ctx := svc.(*AppContext)
	pb.RegisterWishlistServiceServer(s, wishlistgrpc.NewServer(ctx.AppService))
}
...
func initService(cfg any, m *metrics.Metrics) (any, func(), error) {
	...
	// 4. DDD 分层装配
	bootLog.Info("assembling wishlist domain services...")
	repo := persistence.NewWishlistRepository(db)
	wishlistQuery := application.NewWishlistQuery(repo)
	wishlistManager := application.NewWishlistManager(repo, logger.Logger)
	service := application.NewWishlist(wishlistManager, wishlistQuery)
