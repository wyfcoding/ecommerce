...
import (
	...
	wishlistsearch "github.com/wyfcoding/ecommerce/internal/wishlist/infrastructure/persistence/elasticsearch"
	wishlistmysql "github.com/wyfcoding/ecommerce/internal/wishlist/infrastructure/persistence/mysql"
	wishlistredis "github.com/wyfcoding/ecommerce/internal/wishlist/infrastructure/persistence/redis"
	consumer "github.com/wyfcoding/ecommerce/internal/wishlist/interfaces/consumer"
...
const IdempotencyPrefix = "wishlist:idem"

// Config 服务扩展配置
type Config struct {
	configpkg.Config `mapstructure:",squash"`
	Search           struct {
		WishlistIndex string `mapstructure:"wishlist_index" toml:"wishlist_index"`
...
func registerGRPC(s *grpc.Server, ctx *AppContext) {
	pb.RegisterWishlistServiceServer(s, wishlistgrpc.NewServer(ctx.Cmd, ctx.Query))
}
...
func initService(cfg *Config, m *metrics.Metrics) (*AppContext, func(), error) {
	...
	wishlistRepo := wishlistmysql.NewWishlistRepository(db.RawDB())
	wishlistReadRepo := wishlistredis.NewWishlistReadRepository(redisCache.GetClient(), c.Cache.DefaultExpiration)
	wishlistSearchRepo := wishlistsearch.NewWishlistSearchRepository(esClient, c.Search.WishlistIndex)

	// 6.2 Application (Service)
	commandSvc := application.NewWishlistCommandService(wishlistRepo, outbox.NewPublisher(outboxMgr), logger.Logger)
	querySvc := application.NewWishlistQueryService(wishlistRepo, wishlistReadRepo, wishlistSearchRepo)

	// 5.3 Projection Consumers (Wishlist Events -> Read Model)
	projectionService := application.NewWishlistProjectionService(wishlistRepo, wishlistReadRepo, wishlistSearchRepo, logger.Logger)
	projectionHandler := consumer.NewWishlistProjectionHandler(projectionService, logger.Logger)
	projectionTopics := []string{
		domain.WishlistItemAddedEventType,
		domain.WishlistItemRemovedEventType,
		domain.WishlistClearedEventType,
	}
	...
	handler := wishlisthttp.NewHandler(commandSvc, querySvc, logger.Logger)
