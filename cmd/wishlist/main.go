...
import (
	...
	"github.com/wyfcoding/ecommerce/internal/wishlist/infrastructure/messaging"
...
func registerGRPC(s *grpc.Server, ctx *AppContext) {
	pb.RegisterWishlistServiceServer(s, wishlistgrpc.NewServer(ctx.Wishlist))
}
...
func initService(cfg *Config, m *metrics.Metrics) (*AppContext, func(), error) {
	...
	wishlistRepo := persistence.NewWishlistRepository(db.RawDB())

	// 6.2 Application (Service)
	commandSvc := application.NewWishlistCommandService(wishlistRepo, outboxPublisher, logger.Logger)
	querySvc := application.NewWishlistQueryService(wishlistRepo)
	wishlistService := application.NewWishlist(commandSvc, querySvc)

	// 5.3 Interface (HTTP Handlers)
	handler := wishlisthttp.NewHandler(wishlistService, logger.Logger)
