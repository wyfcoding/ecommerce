...
import (
	...
	"ecommerce/internal/content_moderation/handler"
...
// Config 定义了 content_moderation-service 的所有配置项。
type Config struct {
	configpkg.Config
	Data struct {
...
func registerGRPC(s *grpc.Server, srv interface{}) {
	v1.RegisterContentModerationServer(s, srv.(v1.ContentModerationServiceServer))
}

// registerGin 将 HTTP 路由注册到 Gin 引擎。
func registerGin(e *gin.Engine, srv interface{}) {
	handler.RegisterRoutes(e, srv.(*service.ContentModerationService))
}
...
func initService(cfg interface{}, m *metrics.Metrics) (interface{}, func(), error) {
	...
	// 4. 初始化 Repositories
	// content_moderationRepo := repository.NewContentModerationRepo(data)

	// 5. 初始化 Service
	zap.S().Info("initializing content_moderation service...")
	// content_moderationService := service.NewContentModerationService(content_moderationRepo, config.JWT.Secret, config.JWT.Issuer, config.JWT.Expire)

	// TODO: Replace with actual service initialization
	content_moderationService := struct{}{
	}
	...
	return content_moderationService, cleanup, nil
}