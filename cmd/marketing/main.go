...
import (
	...
	marketingmysql "github.com/wyfcoding/ecommerce/internal/marketing/infrastructure/persistence/mysql"
	marketingredis "github.com/wyfcoding/ecommerce/internal/marketing/infrastructure/persistence/redis"
	marketingconsumer "github.com/wyfcoding/ecommerce/internal/marketing/interfaces/consumer"
...
const IdempotencyPrefix = "marketing:idem"

// Config 服务扩展配置
type Config struct {
	configpkg.Config `mapstructure:",squash"`
	Search           struct {
		CampaignIndex string `mapstructure:"campaign_index" toml:"campaign_index"`
...
type AppContext struct {
	Config      *Config
	Command     *application.MarketingCommandService
	Query       *application.MarketingQueryService
	Clients     *ServiceClients
	Handler     *marketinghttp.Handler
...
func registerGRPC(s *grpc.Server, ctx *AppContext) {
	pb.RegisterMarketingServiceServer(s, marketinggrpc.NewServer(ctx.Command, ctx.Query))
}
...
func initService(cfg *Config, m *metrics.Metrics) (*AppContext, func(), error) {
	...
	marketingRepo := marketingmysql.NewMarketingRepository(db.RawDB())
	campaignReadRepo := marketingredis.NewCampaignReadRepository(redisCache.GetClient(), c.Cache.DefaultExpiration)
	bannerReadRepo := marketingredis.NewBannerReadRepository(redisCache.GetClient(), c.Cache.DefaultExpiration)
	campaignSearchRepo := marketingsearch.NewCampaignSearchRepository(esClient, c.Search.CampaignIndex)

	// 5.2 Application (Service)
	publisher := outbox.NewPublisher(outboxMgr)

	query := application.NewMarketingQueryService(marketingRepo, campaignReadRepo, bannerReadRepo, campaignSearchRepo)
	...
	// 5.3 Projection Consumers (Marketing Events -> Read Model)
	projectionService := application.NewMarketingProjectionService(marketingRepo, campaignReadRepo, bannerReadRepo, campaignSearchRepo, logger.Logger)
	projectionHandler := marketingconsumer.NewMarketingProjectionHandler(projectionService, logger.Logger)
	projectionTopics := []string{
		domain.CampaignCreatedEventType,
		domain.CampaignStatusUpdatedEventType,
		domain.ParticipationRecordedEventType,
		domain.BannerCreatedEventType,
		domain.BannerClickedEventType,
		domain.BannerDeletedEventType,
	}
	...
	handler := marketinghttp.NewHandler(command, query, logger.Logger)
