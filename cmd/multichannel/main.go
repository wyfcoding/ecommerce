...
import (
	...
	"github.com/wyfcoding/ecommerce/internal/multichannel/infrastructure/messaging"
	"github.com/wyfcoding/ecommerce/internal/multichannel/infrastructure/persistence"
...
type AppContext struct {
	Config      *Config
	Channel     *application.MultiChannelService
...
func registerGRPC(s *grpc.Server, ctx *AppContext) {
	pb.RegisterMultiChannelServiceServer(s, channelgrpc.NewServer(ctx.Channel))
}
...
func initService(cfg *Config, m *metrics.Metrics) (*AppContext, func(), error) {
	...
	channelRepo := persistence.NewMultiChannelRepository(db.RawDB())

	// 6.2 Application (Service)
	querySvc := application.NewMultiChannelQueryService(channelRepo)
	commandSvc := application.NewMultiChannelCommandService(channelRepo, outboxPublisher, logger.Logger)
	channelService := application.NewMultiChannelService(commandSvc, querySvc)

	// 5.3 Interface (HTTP Handlers)
	handler := channelhttp.NewHandler(channelService, logger.Logger)
