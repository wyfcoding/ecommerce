...
import (
	...
	channelmysql "github.com/wyfcoding/ecommerce/internal/multichannel/infrastructure/persistence/mysql"
	channelredis "github.com/wyfcoding/ecommerce/internal/multichannel/infrastructure/persistence/redis"
	channelconsumer "github.com/wyfcoding/ecommerce/internal/multichannel/interfaces/consumer"
	channelgrpc "github.com/wyfcoding/ecommerce/internal/multichannel/interfaces/grpc"
	channelhttp "github.com/wyfcoding/ecommerce/internal/multichannel/interfaces/http"
...
const BootstrapName = "multichannel"

// IdempotencyPrefix 幂等性 Redis 键前缀
const IdempotencyPrefix = "multichannel:idem"
...
func registerGRPC(s *grpc.Server, ctx *AppContext) {
	pb.RegisterMultiChannelServiceServer(s, channelgrpc.NewServer(ctx.Cmd, ctx.Query))
}
...
func initService(cfg *Config, m *metrics.Metrics) (*AppContext, func(), error) {
	...
	channelRepo := channelmysql.NewMultiChannelRepository(db.RawDB())
	channelReadRepo := channelredis.NewChannelReadRepository(redisCache.GetClient(), c.Cache.DefaultExpiration)
	orderReadRepo := channelredis.NewLocalOrderReadRepository(redisCache.GetClient(), c.Cache.DefaultExpiration)
	orderSearchRepo := channelsearch.NewLocalOrderSearchRepository(esClient, c.Search.OrdersIndex)

	// 6.2 Application (Service)
	publisher := outbox.NewPublisher(outboxMgr)
	querySvc := application.NewMultiChannelQueryService(channelRepo, channelReadRepo, orderReadRepo, orderSearchRepo, logger.Logger)
	commandSvc := application.NewMultiChannelCommandService(channelRepo, publisher, logger.Logger)

	// 5.3 Projection Consumers (MultiChannel Events -> Read Model)
	projectionService := application.NewMultiChannelProjectionService(channelRepo, channelReadRepo, orderReadRepo, orderSearchRepo, logger.Logger)
	projectionHandler := channelconsumer.NewMultiChannelProjectionHandler(projectionService, logger.Logger)
	projectionTopics := []string{
		domain.ChannelRegisteredEventType,
		domain.ChannelOrderCreatedEventType,
	}
	...
	handler := channelhttp.NewHandler(commandSvc, querySvc, logger.Logger)
