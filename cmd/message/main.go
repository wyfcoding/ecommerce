...
const IdempotencyPrefix = "message:idem"
...
func registerGRPC(s *grpc.Server, svc any) {
	ctx := svc.(*AppContext)
	pb.RegisterMessageServiceServer(s, messagegrpc.NewServer(ctx.Message))
}
...
func initService(cfg any, m *metrics.Metrics) (any, func(), error) {
	...
	messageRepo := persistence.NewMessageRepository(db.RawDB())

	// 5.2 Application (Service)
	query := application.NewMessageQuery(messageRepo)
	manager := application.NewMessageManager(messageRepo, logger.Logger)
	messageService := application.NewMessage(manager, query)

	// 5.3 Interface (HTTP Handlers)
	handler := messagehttp.NewHandler(messageService, logger.Logger)
