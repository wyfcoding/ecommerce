...
func registerGin(e *gin.Engine, srv any) {
	...
	// 3. 业务路由
	handler := permissionhttp.NewHandler(ctx.AppService, slog.Default())
	api := e.Group("/api/v1")
...
func initService(cfg any, m *metrics.Metrics) (any, func(), error) {
	...
	// 4. DDD 分层装配
	bootLog.Info("assembling permission services...")
	repo := persistence.NewPermissionRepository(db)
	mgr := application.NewPermissionManager(repo, logger.Logger)
