...
import (
	...
	"github.com/wyfcoding/ecommerce/internal/groupbuy/infrastructure/messaging"
...
type AppContext struct {
	Config      *Config
	Groupbuy    *application.GroupbuyService
...
func registerGRPC(s *grpc.Server, ctx *AppContext) {
	pb.RegisterGroupbuyServiceServer(s, groupbuygrpc.NewServer(ctx.Groupbuy))
}
...
func initService(cfg *Config, m *metrics.Metrics) (*AppContext, func(), error) {
	...
	groupbuyRepo := persistence.NewGroupbuyRepository(db.RawDB())

	// 6.2 Application (Service)
	querySvc := application.NewGroupbuyQueryService(groupbuyRepo)
	commandSvc := application.NewGroupbuyCommandService(groupbuyRepo, outboxPublisher, idGenerator, logger.Logger)
	groupbuyService := application.NewGroupbuyService(commandSvc, querySvc)

	// 5.3 Interface (HTTP Handlers)
	handler := groupbuyhttp.NewHandler(groupbuyService, logger.Logger)
