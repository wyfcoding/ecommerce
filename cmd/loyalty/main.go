...
func registerGRPC(s *grpc.Server, svc any) {
	ctx := svc.(*AppContext)
	pb.RegisterLoyaltyServiceServer(s, loyaltygrpc.NewServer(ctx.Loyalty))
}
...
func initService(cfg any, m *metrics.Metrics) (any, func(), error) {
	...
	loyaltyRepo := persistence.NewLoyaltyRepository(db.RawDB())

	// 5.2 Application (Service)
	query := application.NewLoyaltyQuery(loyaltyRepo)
	manager := application.NewLoyaltyManager(loyaltyRepo, logger.Logger)
	loyaltyService := application.NewLoyalty(manager, query)

	// 5.3 Interface (HTTP Handlers)
	handler := loyaltyhttp.NewHandler(loyaltyService, logger.Logger)
