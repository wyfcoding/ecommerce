...
import (
	...
	"ecommerce/internal/loyalty/data"
...
func main() {
	...
	var conf Config
	if _, err := toml.DecodeFile("../../configs/loyalty.toml", &conf); err != nil {
	...
	err = db.AutoMigrate(&data.UserLoyaltyProfile{}, &data.PointsTransaction{})
	if err != nil {
	...
	loyaltyRepo := data.NewLoyaltyRepo(dataRepo)
	loyaltyUsecase := biz.NewLoyaltyUsecase(loyaltyRepo)
	loyaltyService := service.NewLoyaltyService(loyaltyUsecase)

	log.Println("Application layers wired successfully.")

	// ======== 3. Start the Server (e.g., HTTP, gRPC) ========

	loyaltyhandler.StartHTTPServer(loyaltyService, conf.Service.Port)
