...
import (
	...
	loyaltysearch "github.com/wyfcoding/ecommerce/internal/loyalty/infrastructure/persistence/elasticsearch"
	loyaltymysql "github.com/wyfcoding/ecommerce/internal/loyalty/infrastructure/persistence/mysql"
	loyaltyredis "github.com/wyfcoding/ecommerce/internal/loyalty/infrastructure/persistence/redis"
	loyaltyconsumer "github.com/wyfcoding/ecommerce/internal/loyalty/interfaces/consumer"
...
const IdempotencyPrefix = "loyalty:idem"

// Config 服务扩展配置
type Config struct {
	configpkg.Config `mapstructure:",squash"`
	Search           struct {
		TransactionIndex string `mapstructure:"transaction_index" toml:"transaction_index"`
		BenefitIndex     string `mapstructure:"benefit_index" toml:"benefit_index"`
...
func registerGRPC(s *grpc.Server, ctx *AppContext) {
	pb.RegisterLoyaltyServiceServer(s, loyaltygrpc.NewServer(ctx.Cmd, ctx.Query))
}
...
func initService(cfg *Config, m *metrics.Metrics) (*AppContext, func(), error) {
	...
	loyaltyRepo := loyaltymysql.NewLoyaltyRepository(db.RawDB())
	accountReadRepo := loyaltyredis.NewMemberAccountReadRepository(redisCache.GetClient(), c.Cache.DefaultExpiration)
	benefitReadRepo := loyaltyredis.NewMemberBenefitReadRepository(redisCache.GetClient(), c.Cache.DefaultExpiration)
	txSearchRepo := loyaltysearch.NewPointsTransactionSearchRepository(esClient, c.Search.TransactionIndex)
	benefitSearchRepo := loyaltysearch.NewMemberBenefitSearchRepository(esClient, c.Search.BenefitIndex)

	// 5.2 Application (Service)
	querySvc := application.NewLoyaltyQueryService(loyaltyRepo, accountReadRepo, benefitReadRepo, txSearchRepo, benefitSearchRepo, logger.Logger)
	commandSvc := application.NewLoyaltyCommandService(loyaltyRepo, outbox.NewPublisher(outboxMgr), logger.Logger)

	// 5.3 Projection Consumers (Loyalty Events -> Read Model)
	projectionService := application.NewLoyaltyProjectionService(loyaltyRepo, accountReadRepo, benefitReadRepo, txSearchRepo, benefitSearchRepo, logger.Logger)
	projectionHandler := loyaltyconsumer.NewLoyaltyProjectionHandler(projectionService, logger.Logger)
	projectionTopics := []string{
		domain.MemberAccountUpdatedEventType,
		domain.PointsTransactionCreatedEventType,
		domain.MemberBenefitSavedEventType,
		domain.MemberBenefitDeletedEventType,
	}
	...
	handler := loyaltyhttp.NewHandler(commandSvc, querySvc, logger.Logger)
